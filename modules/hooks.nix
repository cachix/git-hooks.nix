...
let
  ...
  cargoManifestPathArg = lib.optionalString (
  ...
  mkCmdArgs =
    ...
    lib.concatStringsSep " " (
      builtins.foldl' (
        acc: entry: acc ++ lib.optional (builtins.elemAt entry 0) (builtins.elemAt entry 1)
      ) [ ] predActionList
    );

  migrateBinPathToPackage =
    hook: binPath:
    if hook.settings.binPath == null then "${hook.package}${binPath}" else hook.settings.binPath;
in
{
  imports =
    # Rename `settings.<name>.package` to `hooks.<name>.package`
    map (name: mkRenamedOptionModule [ "settings" name "package" ] [ "hooks" name "package" ]) [
      "alejandra"
      "eclint"
      "flynt"
      "mdl"
    ...
    ++
      map
        (
          name: mkRenamedOptionModule [ "settings" name "config" ] [ "hooks" name "settings" "configuration" ]
        )
        [
          "lua-ls"
          "markdownlint"
    ...
    ++ flatten (
      mapAttrsToList
        (name: map (o: mkRenamedOptionModule [ "settings" name o ] [ "hooks" name "settings" o ]))
        {
          "alejandra" = [
            "check"
            "exclude"
    ...
    eslint = mkOption {
      ...
      type = types.submodule {
        ...
        options.settings = {
          ...
          extensions = mkOption {
            type = types.str;
            description = "The pattern of files to run on, see [https://pre-commit.com/#hooks-files](https://pre-commit.com/#hooks-files).";
            default = "\\.js$";
          };
        };
      };
      ...
      type = types.submodule {
        ...
        options.settings = {
          checklevel = mkOption {
            ...
            description = "The diagnostic check level";
            default = "Warning";
          };
          configuration = mkOption {
            type = types.attrs;
            description = "See https://github.com/LuaLS/lua-language-server/wiki/Configuration-File#luarcjson";
            default = { };
          };
        };
      };
    ...
    markdownlint = mkOption {
      ...
      type = types.submodule {
        ...
        options.settings = {
          configuration = mkOption {
            type = types.attrs;
            description = "See https://github.com/DavidAnson/markdownlint/blob/main/schema/.markdownlint.jsonc";
            default = { };
          };
        };
      };
    ...
    ormolu = mkOption {
      ...
      type = types.submodule {
        ...
        options.settings = {
          ...
          cabalDefaultExtensions = mkOption {
            type = types.bool;
    ...
    prettier = mkOption {
      ...
      type = types.submodule {
        ...
        options.settings = {
          ...
          embedded-language-formatting = mkOption {
          ...
          html-whitespace-sensitivity = mkOption {
          ...
          ignore-unknown = mkOption {
            description = "Ignore unknown files.";
            type = types.bool;
            default = true;
          };
          insert-pragma = mkOption {
            description = "Insert @format pragma into file's first docblock comment.";
            type = types.bool;
            default = false;
          };
          jsx-single-quote = mkOption {
          ...
          vue-indent-script-and-style = mkOption {
            description = "Indent script and style tags in Vue files.";
            type = types.bool;
            default = false;
          };
          with-node-modules = mkOption {
            type = types.bool;
    ...
    rumdl = mkOption {
      ...
      type = types.submodule {
        ...
        options.settings = {
          configuration = mkOption {
            type = types.attrs;
            description = "See https://github.com/rvben/rumdl?tab=readme-ov-file#configuration";
            default = { };
            example = {
              configuration = {
                MD013 = {
              ...
              in
              {
                ...
                config = mkOption {
                  ...
                  apply =
                    config:
                    let
                      config' = lib.mapAttrsToList (key: value: "${key}=${toString value}") config;
                    in
                    if config != { } then (builtins.concatStringsSep "," config') else null;
                };
    ...
    typos = mkOption {
      ...
      type = types.submodule {
        ...
        options.settings = {
          ...
          no-check-filenames = mkOption {
            type = types.bool;
            description = "Skip verifying spelling in file names.";
            default = false;
          };

          no-check-files = mkOption {
            type = types.bool;
  ...
  config.warnings =
    ...
    ++
      lib.optional
        (
          cfg.hooks.nixfmt-classic.enable && lib.versionAtLeast cfg.hooks.nixfmt-classic.package.version "1.0"
        )
        ''
  ...
  config.hooks = mapAttrs (_: mapAttrs (_: mkDefault)) {
        ...
        let
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (configPath != null)
                "--experimental-config ${configPath}"
              ]
              [
                (exclude != [ ])
                "--exclude ${lib.strings.concatStringsSep " --exclude " (map lib.escapeShellArg (lib.unique exclude))}"
              ]
              [
                (verbosity == "quiet")
                "-q"
    ...
    ansible-lint = {
      ...
      entry =
        let
          cmdArgs = mkCmdArgs [
            [
              (hooks.ansible-lint.settings.configPath != "")
              "-c ${hooks.ansible-lint.settings.configPath}"
            ]
          ];
        in
        "${hooks.ansible-lint.package}/bin/ansible-lint ${cmdArgs}";
      files =
        if hooks.ansible-lint.settings.subdir != "" then "${hooks.ansible-lint.settings.subdir}/" else "";
    ...
    biome = {
      ...
      entry =
        let
          ...
          cmdArgs = mkCmdArgs [
            [
              (hooks.biome.settings.write)
              "--write"
            ]
            [
              (hooks.biome.settings.configPath != "")
              "--config-path ${hooks.biome.settings.configPath}"
            ]
          ];
        in
    ...
    circleci = {
      ...
      entry = builtins.toString (
        pkgs.writeShellScript "precommit-circleci" ''
          set -e
          failed=false
          for file in "$@"; do
    ...
    cmake-format = {
      ...
      entry =
        let
          maybeConfigPath =
            if
              hooks.cmake-format.settings.configPath == ""
            # Searches automatically for the config path.
            then
              ""
            else
              "-C ${hooks.cmake-format.settings.configPath}";
        in
    ...
    convco = {
      ...
      entry =
        ...
        in
        lib.throwIfNot toolVersionCheck
          "The version of Nixpkgs used by git-hooks.nix does not have the `convco` package (>=0.4.0). Please use a more recent version of Nixpkgs."
          builtins.toString
          script;
      stages = [ "commit-msg" ];
    };
    credo = {
      ...
      entry =
        let
          strict = if hooks.credo.settings.strict then "--strict" else "";
        in
        "${hooks.credo.package}/bin/mix credo ${strict}";
      files = "\\.exs?$";
    };
    crystal = {
      name = "crystal";
        ...
        let
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (exclude != [ ])
                "--exclude ${lib.escapeShellArgs exclude}"
              ]
            ]
          );
        in
    ...
    denofmt = {
      ...
      entry =
        let
          cmdArgs = mkCmdArgs [
            [
              (!hooks.denofmt.settings.write)
              "--check"
            ]
            [
              (hooks.denofmt.settings.configPath != "")
              "-c ${hooks.denofmt.settings.configPath}"
            ]
          ];
        in
    ...
    denolint = {
      ...
      entry =
        let
          cmdArgs = mkCmdArgs [
            [
              (hooks.denolint.settings.format == "compact")
              "--compact"
            ]
            [
              (hooks.denolint.settings.format == "json")
              "--json"
            ]
            [
              (hooks.denolint.settings.configPath != "")
              "-c ${hooks.denolint.settings.configPath}"
            ]
          ];
        in
    ...
    flake8 =
      let
        extendIgnoreStr =
          if lib.lists.length hooks.flake8.settings.extendIgnore > 0 then
            "--extend-ignore " + builtins.concatStringsSep "," hooks.flake8.settings.extendIgnore
          else
            "";
      in
      {
        ...
        let
          ...
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (line-length != null)
                "--line-length ${toString line-length}"
              ]
              [
                no-multiline
                "--no-multiline"
    ...
    fourmolu = {
      ...
      entry = "${hooks.fourmolu.package}/bin/fourmolu --mode inplace ${
        lib.escapeShellArgs (
          lib.concatMap (ext: [
            "--ghc-opt"
            "-X${ext}"
          ]) hooks.fourmolu.settings.defaultExtensions
        )
      }";
      files = "\\.l?hs(-boot)?$";
    };
    ...
    hlint = {
      ...
      package = tools.hlint;
      entry = "${hooks.hlint.package}/bin/hlint${
        if hooks.hlint.settings.hintFile == null then "" else " --hint=${hooks.hlint.settings.hintFile}"
      }";
      files = "\\.l?hs(-boot)?$";
    };
    hpack = {
      ...
      entry = "${hooks.hpack.package}/bin/hpack-dir --${
        if hooks.hpack.settings.silent then "silent" else "verbose"
      }";
        ...
        let
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (rules != [ ])
                "--rules ${lib.strings.concatStringsSep "," rules}"
              ]
              [
                (rulesets != [ ])
                "--rulesets ${lib.strings.concatStringsSep "," rulesets}"
              ]
              [
                show-aliases
                "--show-aliases"
              ...
              [
                (tags != [ ])
                "--tags ${lib.strings.concatStringsSep "," tags}"
              ]
              [
                verbose
                "--verbose"
        ...
        let
          ...
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (indent != null)
                "--indent=${builtins.toString indent}"
              ]
            ]
          );
        in
        lib.throwIf (hooks.nixfmt.settings.indent != null && !hasIndent)
          "`indent` option in `nixfmt` hook can only be used with version >= 1.0.0"
    ...
    nixfmt-classic = {
      ...
      entry =
        let
          ...
          cmdArgs = mkCmdArgs (
            with hooks.nixfmt-classic.settings;
            [
              [
                (width != null)
                "--width=${builtins.toString width}"
    ...
    nixfmt-rfc-style = {
      ...
      entry =
        let
          ...
          cmdArgs = mkCmdArgs (
            with hooks.nixfmt-rfc-style.settings;
            [
              [
                (width != null)
                "--width=${builtins.toString width}"
        ...
        in
        lib.throwIf (hooks.nixfmt-rfc-style.settings.indent != null && !hasIndent)
          "`indent` option in `nixfmt-rfc-style` hook can only be used with version >= 1.0.0"
        ...
        let
          cmdArgs = mkCmdArgs (
            ...
            [
              [
                (branch != [ ])
                "--branch ${lib.strings.concatStringsSep " --branch " branch}"
              ]
              [
                (pattern != [ ])
                "--pattern ${lib.strings.concatStringsSep " --pattern " pattern}"
              ]
            ]
          );
        in
    ...
    ormolu = {
      ...
      entry =
        let
          extensions = lib.escapeShellArgs (
            lib.concatMap (ext: [
              "--ghc-opt"
              "-X${ext}"
            ]) hooks.ormolu.settings.defaultExtensions
          );
          cabalExtensions =
        ...
        let
          ...
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (ignore-path != [ ])
                "--ignore-path ${lib.escapeShellArgs ignore-path}"
              ]
              [
                ignore-unknown
                "--ignore-unknown"
              ...
              [
                (print-width != 80)
                "--print-width ${toString print-width}"
              ]
              [
                (prose-wrap != "preserve")
              ...
              [
                (plugins != [ ])
                "--plugin ${lib.strings.concatStringsSep " --plugin " plugins}"
              ]
              [
    ...
    pretty-format-json = {
      ...
      entry =
        let
          ...
          cmdArgs = mkCmdArgs (
            with hooks.pretty-format-json.settings;
            [
              [
                autofix
                "--autofix"
              ...
              [
                (top-keys != [ ])
                "--top-keys ${lib.strings.concatStringsSep "," top-keys}"
              ]
            ]
          );
        in
      ...
      entry =
        let
          cmdArgs = mkCmdArgs [
            ...
            [
              (hooks.revive.settings.configPath != "")
              "-config ${hooks.revive.settings.configPath}"
            ]
          ];
    ...
    rome = {
      ...
      entry =
        let
          ...
          cmdArgs = mkCmdArgs [
            [
              (hooks.rome.settings.write)
              "--apply"
            ]
            [
              (hooks.rome.settings.configPath != "")
              "--config-path ${hooks.rome.settings.configPath}"
            ]
          ];
        in
        "${binPath} check ${cmdArgs}";
      ...
      let
        cmdArgs = mkCmdArgs (
          ...
          [
            [
              (configuration != { })
              " --config ${toml.generate ".rumdl.toml" configuration}"
            ]
          ]
        );
      in
    ...
    statix = {
      ...
      entry =
        let
          ...
          options = lib.cli.toCommandLine optionFormat (
            lib.mapAttrs (
              _: v: if builtins.isList v then builtins.concatStringsSep " " (lib.unique v) else v
            ) settings
          );
        in
    ...
    typos = {
      ...
      entry =
        let
          ...
          excludeFlags = lib.concatStringsSep " " (lib.map (glob: "--exclude '${glob}'") exclude);
          cmdArgs = mkCmdArgs (
      ...
      settings.config.default.extend-words = lib.mkIf (hooks.typos.settings.ignored-words != [ ]) (
    ...
    woodpecker-cli-lint = {
      ...
      entry =
        let
          cmdArgs = mkCmdArgs (
            with hooks.woodpecker-cli-lint.settings;
            [
              [
                (workflowPath != "")
                workflowPath
              ...
              [
                (pluginsPrivileged != "")
                "--plugins-privileged=${pluginsPrivileged}"
              ]
              [
                (pluginsTrustedClone != "")
                "--plugins-trusted-clone=${pluginsTrustedClone}"
              ]
            ]
          );
        in
        ...
        let
          ...
          cmdArgs = mkCmdArgs (
            ...
            [
              ...
              [
                (configPath != "" && configData == "" && configuration == "" && preset == "default")
