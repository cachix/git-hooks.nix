...
let
  ...
  cfg = config;
  install_stages = lib.unique (
    builtins.concatLists (lib.mapAttrsToList (_: h: h.stages) enabledHooks)
  );
  ...
  enabledExtraPackages = builtins.concatLists (
    mapAttrsToList (_: value: value.extraPackages) enabledHooks
  );
  processedHooks =
    let
      sortedHooks = lib.toposort (a: b: builtins.elem b.id a.before || builtins.elem a.id b.after) (
  ...
  run = mkDerivation {
    ...
    nativeBuildInputs =
      enabledExtraPackages ++ lib.optional (config.settings.rust.check.cargoDeps != null) cargoSetupHook;
  ...
  options = {
    ...
    enabledPackages = mkOption {
      ...
      default = lib.pipe config.hooks [
        ...
        (builtins.concatMap (
          hook: (lib.optional (hook.package != null) hook.package) ++ hook.extraPackages
        ))
      ];
    };
    ...
    hooks = mkOption {
      ...
      description = ''
        ...
        ${concatStringsSep "\n" (
          lib.mapAttrsToList (hookName: hookConf: ''
            **`${hookName}`**
